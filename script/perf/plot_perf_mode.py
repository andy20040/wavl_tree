...
try:
    df = pd.read_csv('../../trace_data/perf/perf_results_modes.csv')
except FileNotFoundError:
    print("Error: perf_results_modes.csv not found")
    exit()
...
for mode in modes:
    ...
    for i, metric in enumerate(metrics):
        metric_data = mode_data[mode_data['Metric'] == metric]
        ...
        ax.set_title(f'[{mode.upper()}] {metric}', fontsize=14, pad=15)
    ...
    filename = os.path.join(output_dir, f'perf_mode_{mode}.png')
